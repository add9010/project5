...
[RequireComponent(typeof(Rigidbody2D), typeof(Animator), typeof(GroundEdgeDetector))]
public class Enemy : MonoBehaviour, IDamageable, IKnockbackable
{
    ...
    public float attackHitDelay = 0.5f; // 애니메이션과 맞춘 히트 딜레이

    [Header("Fall Settings")]
    public float fallGravity = 5f;      // 엣지에서 떨어질 때 중력 스케일

    public GroundEdgeDetector EdgeDetector => edgeDetector;
    #endregion
    ...
    protected virtual void Awake()
    {
        ...
        fallHandler = () => SwitchState(new FallState(fallGravity, rigid.gravityScale));
        edgeDetector.OnEdgeDetected += fallHandler;
    }
    ...
    protected virtual void Update()
    {
        ...
        // 공격 상태 전환
        if (!(currentState is AttackState) && !(currentState is HurtState)
            && IsPlayerInAttackRange() && CanAttack())
        {
